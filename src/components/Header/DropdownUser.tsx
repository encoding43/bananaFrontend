...
const DropdownUser = () => {
  ...
  const hasProfileAccess = useCheckPermission(
    'Profile',
    'name',
  );
      ...
      >
        <span className=" text-right block">
          <span className="block text-sm font-medium text-black dark:text-white capitalize">
            {user?.data?.type === 'contractor'
              ? user?.data?.name
              : user?.data?.ownerName || user?.data?.username }
          </span>
          {/* <span className="block text-xs">({user?.data?.type === 'contractor' ? 'Labor Contractor' :user?.data?.role})</span> */}
          {user?.data?.isAdmin ? (
