...
const LabourBill = () => {
  ...
  // const checkDeletePermission = useCheckPermission('Labor bill', 'delete');
  const checkAddPermission = useCheckPermission('Labor bill', 'Add payment');
  ...
  console.log('formattedLastDateformattedLastDate', formattedLastDate);
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await getAPIAuth(
        `supplier/labor/bill?page=${pageState.pageIndex + 1}&perPage=${pageState.pageSize}&date=${formattedLastDate}&year=${year ? year : ''}&search=${globalFilter == undefined ? '' : globalFilter}&sortBy=${sorting?.length ? sorting?.[0]?.id : ''}&sortOrder=${sorting?.length ? (sorting?.[0]?.desc ? 'desc' : 'asc') : ''}`,
        token,
      );
  ...
  const columns = useMemo(
        ...
        ? [
            {
              ...
              enableSorting: false,
              Cell: ({ row }) => {
                const date = moment(row?.original?.scheduleDate).format(
                  'YYYY-MM-DD',
                );
                if (row?.original?.isSchedule) {
                  return (
                    <WhatsAppComponent
                      phoneNumber={row?.original?.labourMobile}
                      message={`On%20${date}%20we%20promised%20the%20payment%20of%20Rs.${row?.original?.scheduleAmount ? row?.original?.scheduleAmount : row?.original?.amountDue}%20to%20the%20${row?.original?.laborName}.%20Please%20check.
                  `}
                    />
                  );
                } else {
                  return (
                    <WhatsAppComponent
                      phoneNumber={row?.original?.labourMobile}
                      message={`Today%20we%20promised%20the%20payment%20of%20Rs.${row?.original?.scheduleAmount ? row?.original?.scheduleAmount : row?.original?.amountDue}%20to%20the%20${row?.original?.laborName}.%20Please%20check.
                  `}
                    />
                  );
                }
    ...
    [pageState.pageIndex, activeInputs, inputValues , checkNotificationPermission,checkAddPermission,checkPaymentReminderPermission],
  );
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        <Breadcrumb pageName="Labour Bill" />
      </div>
...
export default withPageGuard(LabourBill, 'Labor bill', 'List');
