...
import AddExceptionModal from './ExceptionText.jsx';
...
const TaskManagement = () => {
  ...
  const [editExceptionHarvestingdata, setEditExceptionHarvestingData] = useState({});
  ...
  const hasPermission = useCheckPermission('Home', 'add task');
  const hasPermissionAssigned = useCheckPermission('Home', 'assign');
  const hasPermissionNote = useCheckPermission('Home', 'note');
  const hasPermissionDelete = useCheckPermission('Home', 'delete');
  const hasPermissionEdit = useCheckPermission('Home', 'edit');
  const hasPermissionViewTask = useCheckPermission('Home', 'view task');
console.log("hasPermissionEdithasPermissionEdit" , hasPermission);


  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await getAPIAuth(`task/allTask?page=${pageState.pageIndex + 1}&perPage=${pageState.pageSize}&sortBy=${sorting?.length ? sorting?.[0]?.id : ''}&name=${globalFilter == undefined ? "" :  globalFilter}&sortOrder=${sorting?.length ? (sorting?.[0]?.desc ? 'desc' : 'aesc') : ''}`, token);
    ...
    () => [
        {
          ...
          },
          {
            ...
            enableSorting: true,
            Cell: ({ row }) => {
              // console.log("sdkjfhksdjljfkds" , row);
              const deadLine = row.original.deadLine ? row.original.deadLine : "dsddsd";
          ...
          },
          {
            ...
            Cell: ({ cell, row }) => {
              ...
              const bgColor = row?.original?.color || 'transparent';
              ...
              return (
                ...
                <span className='size-4 ' style={{ backgroundColor: bgColor, padding: '6px', borderRadius: '40px', display: 'inline-block', }}>
                </span>
                </>
              );
            },
        ...
        Cell: ({ row }) => (
          <Dropdown>
            ...
            <Menu slots={{ listbox: Listbox }} className="z-99">
              ...
              <MenuItem onClick={() => handleAssignTask(row.original)}>Assign</MenuItem>
              <MenuItem onClick={() => handleAssignViewTask(row.original)}> View Task</MenuItem>
              <MenuItem 
                sx={{
                  backgroundColor: row?.original?.note?.length > 0 ? 'green' : 'initial',
                  color: row?.original?.note?.length > 0 ? '#fff' : 'initial',
                  '&:hover': {
                    backgroundColor: row?.original?.note?.length > 0 ? 'darkgreen' : 'initial',
                    color: row?.original?.note?.length > 0 ? '#fff' : 'initial',
                  },
                }}
  onClick={() => handleExceptionFarmer(row.original)}>Note</MenuItem>
    ...
    [pageState?.pageIndex , hasPermission , hasPermissionAssigned , hasPermissionNote , hasPermissionDelete , hasPermissionEdit , hasPermissionViewTask]
  );
  ...
  const handleAssignViewTask = (task) => {
    if(hasPermissionViewTask){
    setAssignView(task);
    setOpenViewTaskAddModal(true);
    }
  };
  const handleAssignTask = (task) => {
    if(hasPermissionAssigned){
      setEditingTask(task);
  ...
  const handleDeleteFarmer = async (id) => {
   if(hasPermissionDelete){
    try {
      const response = await getAPIAuth(`task/deleteTask?id=${id}` , "Home" , "delete");
  ...
  return (
    <>
      ...
      />
       <AddExceptionModal openAddModal={openExceptionAddModal} 
       setOpenAddModal={setOpenExceptionAddModal} 
       editHarvestingdata={editExceptionHarvestingdata} 
       getFarmers={fetchData}/>
    </>
  );
};

export default withPageGuard(TaskManagement , "Home" , "task list");
