...
import AddCommissionAgentModal from './AddCommissionAgentsModal.jsx';
import ExportButtonContent from '../../utils/ExportButtonContent.jsx';

const AdddCommissionAgents = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await getAPIAuth(
        `commissionAgent/getCommissionAgentDetail?page=${pageState.pageIndex + 1}&perPage=${pageState.pageSize}&search=${globalFilter == undefined ? '' : globalFilter}&sortBy=${sorting?.length ? sorting?.[0]?.id : ''}&sortOrder=${sorting?.length ? (sorting?.[0]?.desc ? 'desc' : 'aesc') : ''}`,
        token,
      );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Commission Agent Address',
        size: 200,
        enableColumnActions: false,
        enableSorting: false,
      },
      {
        ...
        header: 'Commission Agent Short form/ Alias',
        size: 200,
        enableColumnActions: false,
        enableSorting: true,
      },
  ...
  return (
    <>
      ...
      <div className="flex justify-between items-start sm:items-center mb-6 gap-3 flex-col sm:flex-row">
        <Breadcrumb pageName="View CommissionAgent" />

        <div className="flex gap-3">
          {checkExportPermission ? (
            <>
              <ExportButtonContent apikey="commissionAgent/getCommissionAgentDetail" />
            </>
          ) : (
            ''
          )}
...
export default withPageGuard(AdddCommissionAgents, 'Manage Company', 'List');
