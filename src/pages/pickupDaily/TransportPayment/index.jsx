...
const PickUpTransportPaymet = () => {
  ...
  const checkAddPaymentPermission = useCheckPermission('Pickup payment', 'Add Payment');
  // const checkExportPermission = useCheckPermission('Pickup payment', 'Pay.details Export');
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await getAPIAuth(
        `transport/transportBill?page=${pageState.pageIndex + 1}&perPage=${pageState.pageSize}&year=${year}&month=${monthNumber}&search=${globalFilter == undefined ? '' : globalFilter}&sortBy=${sorting?.length ? sorting?.[0]?.id : ''}&transportType=pickUp&sortOrder=${sorting?.length ? (sorting?.[0]?.desc ? 'desc' : 'asc') : ''}`,
        token,
      );
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        <Breadcrumb pageName="Pickup Transport Payment" />
      </div>
      <Box
        display="flex"
        justifyContent="end"
...
export default withPageGuard(PickUpTransportPaymet , "Pickup payment" , "List" );
