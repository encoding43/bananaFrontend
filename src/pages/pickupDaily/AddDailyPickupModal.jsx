...
export default function AddDailyPickupModal({
  openAddModal,
  setOpenAddModal,
  editFarmerData,
  setEditFarmerData,
  ...
  const initialValues = {
    type: 'pickUp',
    pickUpNo: editFarmerData?.pickUpNo || '',
  ...
  const formik = useFormik({
    ...
    validationSchema: Yup.object({
      pickUpNo: Yup.number().required('pickUpNo is required'),
    ...
    onSubmit: async (values, { resetForm }) => {
      ...
      formData.append('pickUpNo', values?.pickUpNo);
      ...
      try {
        ...
        if (editFarmerData) {
          ...
          res = await postAPIAuthFormData(
            `transport/editTransportDetail`,
            formData,
          );
        } else {
          res = await postAPIAuthFormData(
            `transport/addTransportDetail`,
            formData,
          );
        }
  ...
  return (
    <React.Fragment>
      ...
      >
        <DialogTitle>
          {editFarmerData ? 'Edit pickup' : 'Add Daily pickup'}
        </DialogTitle>
        <IconButton
          aria-label="close"
          onClick={handleClose}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  type="text"
                  name="pickUpNo"
                  placeholder="Enter Pickup No"
                  value={formik.values.pickUpNo}
                  onChange={formik.handleChange}
                  onBlur={formik.handleBlur}
                  className={`w-full rounded-lg border-[1.5px] border-stroke bg-transparent px-5 py-3 font-normal text-black outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:text-white dark:focus:border-primary ${
                    formik.touched.pickUpNo && formik.errors.pickUpNo
                      ? 'border-red-500'
                      : ''
                  }`}
                />
