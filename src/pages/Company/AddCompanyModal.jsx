...
export default function AddCompanyModal({
  ...
  const validationSchema = Yup.object({
    ...
    companyShortForm: Yup.string().required(
      'Company Short form/ Alias is required',
    ),
  ...
  const handleFormSubmit = async (vlaues, resetForm) => {
    ...
    try {
      const userData = {
        companyName: vlaues?.companyName,
        companyAddress: vlaues?.companyAddress,
        companyAliasName: vlaues?.companyShortForm,
        companyEmail: vlaues?.companyEmail,
      ...
      if (editingLabor) {
        res = await postAPIAuth(
          `supplier/company/update/${editingLabor._id}`,
          userData,
        );
      } else {
        res = await postAPIAuth('/supplier/company/create', userData);
      }
      if (res?.data?.success) {
        toast.success(
          `Company ${editingLabor ? 'updated' : 'added'} successfully.`,
        );
        handleClose();
        updateData();
        resetForm();
      ...
      toast.error(
        `Error ${editingLabor ? 'updating' : 'adding'} company. Please try again later.`,
      );
    }
  };
  ...
  const formik = useFormik({
    initialValues: {
      companyName: editingLabor?.companyName || '',
      companyAddress: editingLabor?.companyAddress || '',
      companyShortForm: editingLabor?.companyAliasName || '',
      companyEmail: editingLabor?.companyEmail || '',
  ...
  return (
    <React.Fragment>
      ...
      >
        <DialogTitle>
          {' '}
          {editingLabor ? 'Edit Company' : 'Add Company'}
        </DialogTitle>
        <IconButton
          aria-label="close"
          onClick={handleClose}
