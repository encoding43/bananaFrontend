  ...
  const fetchData = async () => {
    ...
    try {
      const res = await getAPIAuth(
        `/supplier/company/get?page=${pageState.pageIndex + 1}&perPage=${pageState.pageSize}&search=${globalFilter == undefined ? '' : globalFilter}&sortBy=${sorting?.length ? sorting?.[0]?.id : ''}&sortOrder=${sorting?.length ? (sorting?.[0]?.desc ? 'desc' : 'aesc') : ''}`,
        token,
      );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: 'Company Short form/ Alias',
        size: 200,
        enableColumnActions: false,
        enableSorting: true,
      },
  ...
  return (
    <>
      ...
      <div className="flex justify-between items-start sm:items-center mb-6 gap-3 flex-col sm:flex-row">
        <Breadcrumb pageName="Manage Company" />

        <div className="flex gap-3">
          {checkExportPermission ? (
            <>
              <ExportButtonContent apikey="supplier/company/get" />
            </>
          ) : (
            ''
          )}
...
export default withPageGuard(Company, 'Manage Company', 'List');
