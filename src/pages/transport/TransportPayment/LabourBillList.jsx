...
export default function LabourBillList({
  ...
  const fetchData = async () => {
    try {
      const res = await getAPIAuth(
        `transport/transportBill?vehicleNo=${party?._id}&page=${pageState.pageIndex + 1}&perPage=${pageState.pageSize}&transportType=dailyWeight&search=${globalFilter == undefined ? '' : globalFilter}&sortBy=${sorting?.length ? sorting?.[0]?.id : ''}&sortOrder=${sorting?.length ? (sorting?.[0]?.desc ? 'desc' : 'asc') : ''}`,
        token,
      );
  ...
  const apiKey = `transport/transportBill?vehicleNo=${party?._id}&transportType=dailyWeight`;
  return (
    <Dialog
      open={openAddModal}
      onClose={handleClose}
