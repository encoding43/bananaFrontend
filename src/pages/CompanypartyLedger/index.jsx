...
import CompanypartyLedgerModal from './CompanypartyLedgerModal';
...
const CompanypartyLedger = () => {
  ...
  console.log("qqqtotalsAmount" , totalsAmount);
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await getAPIAuth(
        `supplier/company/companyLedger?page=${pageState.pageIndex + 1}&perPage=${pageState.pageSize}&date=${formattedLastDate}&year=${year ? year : "" }&search=${globalFilter == undefined ? "" :  globalFilter}&sortBy=${sorting?.length ? sorting?.[0]?.id : ''}&sortOrder=${sorting?.length ? (sorting?.[0]?.desc ? 'desc' : 'asc') : ''}`,
        token
      );
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        enableSorting: false,
        Cell: ({ row }) => (
          <button className='text-blue-600 hover:font-medium capitalize'  onClick={() => handlePartyClick(row.original)}> 
  ...
  return (
    <>
      <div className="flex justify-between items-center mb-6">
        <Breadcrumb pageName="Company/Party Ledger" />
      </div>
...
export default withPageGuard(CompanypartyLedger , "Company Ledger" , "List" );
