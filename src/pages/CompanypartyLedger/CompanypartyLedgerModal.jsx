...
export default function CompanypartyLedgerModal({
  openAddModal,
  setOpenAddModal,
  party,
}) {
  console.log('totalsAmounttotalsAmount', party);
  ...
  const checkExportPermission = useCheckPermission('Company Ledger', 'export');
  ...
  const fetchData = async () => {
    try {
      const res = await getAPIAuth(
        `supplier/company/companyLedger?companyId=${party?._id}&page=${pageState.pageIndex + 1}&perPage=${pageState.pageSize}&search=${globalFilter == undefined ? '' : globalFilter}&sortBy=${sorting?.length ? sorting?.[0]?.id : ''}&sortOrder=${sorting?.length ? (sorting?.[0]?.desc ? 'desc' : 'asc') : ''}`,
        token,
      );
  ...
  }, [party, pageState.pageIndex, globalFilter, sorting, party]);
  ...
  const handleCheckboxChange = async (e, item) => {
    ...
    try {
      const res = await getAPIAuth(
        `supplier/company/companyLedgerFalse?id=${item._id}&status=${isChecked}`,
        token,
      );
  ...
  const apikey = `supplier/company/companyLedger?companyId=${party?._id}`;
  return (
    <Dialog
      open={openAddModal}
      onClose={handleClose}
    ...
    >
      <div className="pt-10 w-[calc(100%_-_45px)] flex justify-between pr-4 items-start sm:items-center mb-6 gap-3 flex-col sm:flex-row">
        <DialogTitle>Company/Party Ledger Details</DialogTitle>
      ...
      <DialogContent>
        ...
        <div className="table-container capitalize mt-5">
          <MaterialReactTable
            columns={columns}
            data={data}
