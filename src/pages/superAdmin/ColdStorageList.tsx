...
const ColdStorageTable = () => {
  ...
  const getSupplierList = async () => {
    try {
      const res = await getAPIAuth(
        `admin/coldStoreList?page=${pageState.pageIndex + 1}&perPage=${pageState.pageSize}&search=${globalFilter == undefined ? "" :  globalFilter}&sortBy=${sorting?.length ? sorting?.[0]?.id : ''}&sortOrder=${sorting?.length ? (sorting?.[0]?.desc ? 'desc' : 'aesc') : ''}`,token
      );
      if (res?.data?.success) {
  ...
  const SwitchhandleChange = async (row) => {
    try {
      const res = await getAPIAuth(`admin/blockUnblockColdStore?id=${row._id}&block=${!row.isBlocked}`, token);
  ...
  return (
    <>
        <div className="flex justify-between items-start sm:items-center mb-6 gap-3 flex-col sm:flex-row">
          <Breadcrumb pageName="Cold Storage List" />
          <ExportButtonContent
           apikey = "admin/coldStoreList"
           />
          {/* <button 
...
// withPageGuard(ColdStorageTable , "Super Admin" , "View coldStorageList" )