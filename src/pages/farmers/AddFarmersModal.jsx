...
export default function AddFarmersModal({openAddModal, setOpenAddModal , editHarvestingdata , getFarmers}) {
  ...
  const [rows, setRows] = React.useState([{ boxKgType: '', customeVallue : '' , boxweight: '' , count: '', brand: '' , id: '', handsRatio : {
    '3H' : '',
    '4H' : '',
    '5H' : '',
    '6H' : '',
  ...
  console.log("packagingMaterialData" , userDetails);
  let isError = false;
  ...
  console.log("formValuesformValues" , formValues?.box);
...
React.useEffect(() => {
  ...
  const handleClose = () => {
    ...
    setRows([{ boxKgType: '', customeVallue : '' , boxweight: '' , count: '', id: '', brand: '' , handsRatio : {
      '3H' : '',
      '4H' : '',
      '5H' : '',
      '6H' : '',
  ...
  const handleAddRow = () => {
    setRows([...rows, { boxKgType: '', customeVallue : '' , boxweight : '' , count: '', brand: '', id: '' , handsRatio : {
      '3H' : '',
      '4H' : '',
      '5H' : '',
      '6H' : '',
...
// Call the function to check for duplicates
  const handleFormSubmit = async (e)=> {
    ...
    if (userDetails?.data?.role == "FruitSupplier" || userDetails?.data?.role == "accountant") {
       ...
       if(formValues.laborId && formValues.vehicleNo && formValues.companyId && formValues.rate && formValues.coldStoreId &&formValues.date && !isError ){
        ...
        if(HandRatioHandelStop == undefined){
          ...
          if(res.status == 200){
            ...
            setRows([{ boxKgType: '', customeVallue : '' , boxweight : '' , count: '', brand: '' ,id: '', handsRatio : {
              '3H' : '',
              '4H' : '',
              '5H' : '',
              '6H' : '',
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <DialogContent>
          <div className="flex flex-wrap sm:grid grid-cols-1 sm:grid-cols-2 gap-4 max-sm:*:w-full">
            {/* <div>
...
<div>
  ...
  {/* <div className='col-span-1'>
    ...
    <Autocomplete
      value={coldStorageList?.find((option) => option._id === formValues.coldStoreId) || null}
      onChange={(e) => {
      ...
      renderInput={(params) => (
        <TextField
          ...
          sx={{ width: '100%', height: '49.6px' }}
          error={!!inputErrors.coldStoreId}
          helperText={inputErrors.coldStoreId}
        />
      )}
      sx={{ width: '100%' }}
    />
...
<div className='col-span-1'> 
  ...
  <Autocomplete
    value={coldStorageList?.find((option) => option._id === formValues.coldStoreId) || null}
    onChange={(e, newValue) => {
      // Handle value change from Autocomplete
      setFormValues((prev) => ({
        ...prev,
        coldStoreId: newValue ? newValue._id : '',
      }));
      setInputErrors((prev) => ({
        ...prev,
        coldStoreId: '',  // Clear error when user selects a value
      }));
    }}
    onBlur={() => {
      ...
      if (!formValues.coldStoreId) {
        setInputErrors((prev) => ({
          ...prev,
          coldStoreId: 'Cold Storage is required',
        }));
      }
    }}
    options={coldStorageList}
  ...
  {/*  */}
    ...
    {inputErrors.coldStoreId && (
      <p className="text-red-500 text-sm mt-1">{inputErrors.coldStoreId}</p>
          ...
          {
            userDetails?.data?.role == "FruitSupplier" || userDetails?.data?.role == "accountant"  ? (
              <>
                <div>
              <label className="mb-3 block text-sm font-medium text-black dark:text-white">
                Company Rate
            ...
            <div>
              ...
              <input multiple onChange={(e)=>setFormValues(prev=>({...prev, images: Array.from(e.target.files)}))} type="file" accept=".jpg, .jpeg, .png" className="w-full cursor-pointer rounded-lg border-[1.5px] border-stroke bg-transparent font-normal outline-none transition file:mr-5 file:border-collapse file:cursor-pointer file:border-0 file:border-r file:border-solid file:border-stroke file:bg-whiter file:px-5 file:py-3 file:hover:bg-primary file:hover:bg-opacity-10 focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:file:border-form-strokedark dark:file:bg-white/30 dark:file:text-white dark:focus:border-primary"/>
            </div>
      ...
      console.log("dkljdsfkldjldsdjl" , formValues?.box );
        ...
        return(
          <>
          <Box className="flex-wrap md:flex-nowrap" key={index} display="flex" alignItems="center" gap={2} mb={index === rows.length - 1 ? '' : 2}>
            ...
            <>
              <div className="w-[47%] md:w-[calc(30%_-_40px)]">
