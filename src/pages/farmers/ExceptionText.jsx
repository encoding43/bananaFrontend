...
export default function AddExceptionModal({ openAddModal, setOpenAddModal, editHarvestingdata, getFarmers }) {
    ...
    const validationSchema = Yup.object({
        exceptionDetails: Yup.string().required('Exception details are required'),
    });
    ...
    const getExeptionFarmers = async(values , resetForm ) => {
        ...
        const param = {
            "exceptionDetails" : values.exceptionDetails , 
            "id" : editHarvestingdata._id
        }

        try {
            const res = await postAPIAuth(`farmer/exceptionDetails`, param);
    ...
    const formik = useFormik({
        initialValues: {
            exceptionDetails: editHarvestingdata?.exceptionDetails || '',
        },
